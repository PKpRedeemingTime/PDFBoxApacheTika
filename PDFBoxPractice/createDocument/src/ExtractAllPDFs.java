...
public class ExtractAllPDFs {
	...
	private static void extractFiles(Map<String, PDComplexFileSpecification> names, String targetDir) throws IOException {
        for (Map.Entry<String, PDComplexFileSpecification> entry : names.entrySet()) {
        	if(entry.getKey().contains(".zip")) {
                ...
                copyFileToDirectory(fileName, embeddedFile, targetDir);
        	}
        }
    }
	...
	private static void copyFileToDirectory(final String srcFile, final PDEmbeddedFile embeddedFile, String destDirectory) throws IOException {
		String temp = srcFile.substring(0, srcFile.lastIndexOf('.')) + ".zip";
